...
/**
 * @title CUIDAQUANNFT
 * @dev ERC721标准的NFT合约，支持铸造功能，用于测试白名单购买
 */
contract CUIDAQUANNFT is ERC721, ERC721URIStorage, Ownable {
     ...
     * @dev 构造函数
     */
    constructor() ERC721("CUIDAQUAN NFT", "CQNFT") Ownable(msg.sender) {
        // NFT名称: CUIDAQUAN NFT, 符号: CQNFT
        // 初始化计数器为0
        _tokenIdCounter = 0;
    }
    ...
    /**
     * @dev 批量铸造NFT - 只有合约拥有者可以调用
     * @param to 接收者地址
     * @param uris NFT的元数据URI数组
     * @return tokenIds 新铸造的NFT的ID数组
     */
    function batchMint(address to, string[] memory uris) public onlyOwner returns (uint256[] memory) {
        require(to != address(0), "Cannot mint to zero address");
        require(uris.length > 0, "URI array cannot be empty");
        require(uris.length <= 20, "Cannot mint more than 20 NFTs at once");
