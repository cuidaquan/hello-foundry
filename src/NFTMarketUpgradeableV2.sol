 ...
 * @dev 可升级的 NFT 市场合约 V2，在 V1 基础上添加离线签名上架功能
 */
contract NFTMarketUpgradeableV2 is
    Initializable,
    IERC721Receiver,
    ITokenReceiver,
    ...
    UUPSUpgradeable
{
    ...
    // 市场手续费率 (基点，10000 = 100%)
    uint16 public marketFeeRate;

    // 累计的市场手续费
    uint256 public accumulatedFees;

    // V2 新增状态变量
    // 签名 nonce 映射，防止重放攻击 (seller => tokenId => nonce)
    mapping(address => mapping(uint256 => uint256)) public signatureNonces;
    ...
    /**
     * @dev 通过离线签名上架 NFT（新功能）
     * @param tokenId NFT ID
     * @param price 价格
     * @param nonce 签名 nonce
    ...
    function listWithSignature(
        ...
        require(signatureNonces[seller][tokenId] == nonce, "Invalid nonce");

        // 更新 nonce，防止重放攻击
        unchecked {
            signatureNonces[seller][tokenId]++;
        }
        ...
        unchecked {
            ...
            emit NFTListedWithSignature(listingId, seller, nftAddr, tokenId, price, nonce);
        }
    }
     ...
     * @param tokenId NFT ID
     */
    function getSignatureNonce(address seller, uint256 tokenId) external view returns (uint256) {
        return signatureNonces[seller][tokenId];
    }
